...
class AlunoDAO{
  ...
  public function buscarUmAluno($nome){
    try {

      $query = "SELECT * FROM aluno WHERE nome_alu = '{$nome}' ";
      $resultado = pg_query($this->db->getConexao() , $query);
      
      $aluno = null;
      while($alunoDAO = pg_fetch_assoc($resultado)){
        $nome = $alunoDAO['nome_alu'];
        $cidade = $alunoDAO['cidade_alu'];
        $telefone = $alunoDAO['tel_alu'];
        $sexo = $alunoDAO['sexo_alu'];
        $email = $alunoDAO['email_alu'];

        $aluno = new Aluno($nome,$cidade,$telefone,$sexo,$email);
      }

      // echo "AlunoDAO > lisbuscarUmAlunotar() ->  " . $aluno->getNome();
      $this->db->desconectar();
      return $aluno;
    }catch(err){
      echo "AlunoDAO > listarUmAluno() > Deu ruim =( - " . $err;
      return $err;
    }
  }
